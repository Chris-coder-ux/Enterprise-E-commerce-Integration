<?php
/**
 * Script de limpieza para productos WooCommerce problemáticos - VERSIÓN MEJORADA
 * Elimina productos que no se muestran correctamente en lotes para evitar timeouts
 *
...
class WooCommerceProductCleaner
{
    ...
    /**
     * Obtener lista detallada de productos problemáticos con paginación
     */
    public function getProblematicProductsList(int $limit = 100, int $offset = 0): array {
        global $wpdb;
        
        $problematic_products = [];
        
        // 1. Productos publicados pero con precio 0
        $products_price_zero = $wpdb->get_results($wpdb->prepare("
        ...
        // 2. Productos sin SKU
        $products_no_sku = $wpdb->get_results($wpdb->prepare("
        ...
        $products_bad_name = $wpdb->get_results($wpdb->prepare("
            ...
            WHERE p.post_type = 'product' 
            AND p.post_status = 'publish'
            AND (p.post_title IS NULL OR p.post_title = '' OR p.post_title = 'Auto Draft' OR p.post_title LIKE 
'Producto VERIAL_%')
            ORDER BY p.post_date DESC 
            LIMIT %d OFFSET %d
        ", $limit, $offset));
        ...
        foreach ($problematic_statuses as $status) {
            $products_wrong_status = $wpdb->get_results($wpdb->prepare("
                SELECT p.ID, p.post_title, p.post_status, pm_sku.meta_value as sku, pm_price.meta_value as 
price
                FROM {$wpdb->posts} p 
                LEFT JOIN {$wpdb->postmeta} pm_sku ON p.ID = pm_sku.post_id AND pm_sku.meta_key = '_sku'
                LEFT JOIN {$wpdb->postmeta} pm_price ON p.ID = pm_price.post_id AND pm_price.meta_key = 
'_price'
                WHERE p.post_type = 'product' 
                AND p.post_status = %s
                ORDER BY p.post_date DESC 
        ...
        $verial_problematic = $wpdb->get_results($wpdb->prepare("
            SELECT p.ID, p.post_title, p.post_status, pm_sku.meta_value as sku, pm_price.meta_value as price
            FROM {$wpdb->posts} p 
            LEFT JOIN {$wpdb->postmeta} pm_sku ON p.ID = pm_sku.post_id AND pm_sku.meta_key = '_sku'
            LEFT JOIN {$wpdb->postmeta} pm_price ON p.ID = pm_price.post_id AND pm_price.meta_key = '_price'
    ...
    /**
     * Eliminar productos problemáticos de forma segura en lotes
     */
    public function safeCleanup(array $product_ids, bool $dry_run = true, int $batch_size = 20): array {
        $results = [
        ...
        // Aumentar límites para operaciones masivas
        @ini_set('max_execution_time', 0);
        @ini_set('memory_limit', '512M');
        
        // Para mejorar el rendimiento y reducir el uso de memoria
        ...
        // Guardar los productos restantes para procesar en la próxima ejecución
        $results['remaining_products'] = array_merge(...$batches);
        ...
        // Limpiar caché y recolectar ciclos
        wp_cache_flush();
    ...
    private function productHasOrders(int $product_id): bool {
        ...
        $order_count = $wpdb->get_var($wpdb->prepare("
            ...
            LEFT JOIN {$wpdb->prefix}woocommerce_order_itemmeta AS meta ON items.order_item_id = 
meta.order_item_id
    ...
    /**
     * Limpieza masiva por tipo de problema con paginación
     */
    public function cleanupByIssueType(string $issue_type, bool $dry_run = true, int $batch_size = 20): array 
{
        $all_problematic = $this->getProblematicProductsList(500);
        
        $filtered_products = array_filter($all_problematic, function($product) use ($issue_type) {
            return $product['type'] === $issue_type;
        });
        
        $product_ids = array_column($filtered_products, 'id');
        
        return $this->safeCleanup($product_ids, $dry_run, $batch_size);
    }
    ...
    /**
     * Eliminación masiva de TODOS los productos en lotes
     */
    public function massDeleteAllProducts(bool $dry_run = true, int $batch_size = 20): array {
        $all_products = $this->getAllProducts(500);
...
function run_product_cleanup_script() {
    ...
    echo "<h1>🧹 Limpiador de Productos WooCommerce - VERSIÓN MEJORADA</h1>";
    echo "<p><strong>⚠ ADVERTENCIA:</strong> Este script elimina productos en lotes pequeños para evitar 
errores. Haz una copia de seguridad primero.</p>";
    echo "<div style='background: #e7f3ff; border: 1px solid #b3d9ff; padding: 10px; margin: 10px 0; 
border-radius: 4px;'>";
    echo "<strong>ℹ CÓMO FUNCIONA:</strong> El script procesa solo <strong>20 productos por 
ejecución</strong>. Si hay más productos, ejecuta el script nuevamente para procesar los siguientes.";
    echo "</div>";
    ...
    $problematic_list = $cleaner->getProblematicProductsList(10);
    
    echo "<h2>📋 Productos Problemáticos (primeros 10)</h2>";
    ...
    $all_products_list = $cleaner->getAllProducts(10);
    
    echo "<h2>📦 TODOS los Productos (primeros 10)</h2>";
    echo "<div style='background: #e7f3ff; border: 1px solid #b3d9ff; padding: 10px; margin: 10px 0; 
border-radius: 4px;'>";
    echo "<strong>ℹ INFORMACIÓN:</strong> Esta tabla muestra solo los primeros 10 productos. ";
    echo "Si usas la opción de 'Eliminación Masiva Completa', se procesarán en lotes de 20.";
    echo "</div>";
    echo "<table border='1' cellpadding='8' style='border-collapse: collapse; width: 100%;'>";
    echo "<tr>
            <th>ID</th>
    ...
    foreach ($all_products_list as $product) {
        echo "<tr>
                ...
                <td>{$product['status']}</td>
                <td>{$product['price']}</td>
                <td>Se eliminará en lotes</td>
              </tr>";
    }
    echo "</table>";
    
    // Opciones de limpieza
    echo "<h2>🛠 Opciones de Limpieza</h2>";
    ...
    echo "</select>";
    echo "<br><br>";
    echo "<label><input type='checkbox' name='dry_run' value='1' checked> <strong>Modo simulación (solo 
mostrar qué se eliminaría)</strong></label>";
    echo "<br><br>";
    echo "<div style='background: #fff3cd; border: 1px solid #ffeaa7; padding: 10px; margin: 10px 0; 
border-radius: 4px;'>";
    echo "<strong>⚠ IMPORTANTE:</strong> Con el modo simulación activado, NO se eliminarán productos. 
Desmarca la casilla para eliminar realmente.";
    echo "</div>";
    echo "<input type='submit' name='cleanup_by_type' value='Ejecutar Limpieza por Tipo' style='background: 
#dc3232; color: white; padding: 10px; border: none; cursor: pointer;'>";
    echo "</form>";
    
    echo "<form method='post'>";
    echo "<h3>Limpieza Masiva</h3>";
    echo "<label><input type='checkbox' name='dry_run' value='1' checked> <strong>Modo 
simulación</strong></label>";
    echo "<br><br>";
    echo "<div style='background: #fff3cd; border: 1px solid #ffeaa7; padding: 10px; margin: 10px 0; 
border-radius: 4px;'>";
    echo "<strong>⚠ IMPORTANTE:</strong> Con el modo simulación activado, NO se eliminarán productos. 
Desmarca la casilla para eliminar realmente.";
    echo "</div>";
    echo "<input type='submit' name='cleanup_all' value='Limpiar TODOS los productos problemáticos' 
style='background: #dc3232; color: white; padding: 10px; border: none; cursor: pointer;'>";
    echo "</form>";
...
border-radius: 4px;'>";
    echo "<strong>⚠ PELIGRO:</strong> Esta opción eliminará <strong>TODOS</strong> los productos de 
WooCommerce, sin excepción. ";
    echo "Solo úsala si estás seguro de que quieres limpiar completamente la tienda.";
    echo "</div>";
    echo "<label><input type='checkbox' name='dry_run' value='1' checked> <strong>Modo 
simulación</strong></label>";
    echo "<br><br>";
    echo "<label><input type='checkbox' name='confirm_mass_delete' value='1'> <strong>Confirmo que quiero 
eliminar TODOS los productos</strong></label>";
    echo "<br><br>";
    echo "<input type='submit' name='mass_delete_all' value='🚨 ELIMINAR TODOS LOS PRODUCTOS' 
style='background: #721c24; color: white; padding: 15px; border: none; cursor: pointer; font-weight: bold;'>";
    echo "</form>";
    ...
    if (isset($_POST['cleanup_by_type']) || isset($_POST['cleanup_all']) || isset($_POST['mass_delete_all'])) 
{
        ...
        $batch_size = 20; // Tamaño del lote para evitar timeouts
        ...
        if (isset($_POST['mass_delete_all'])) {
            ...
            if (!isset($_POST['confirm_mass_delete']) || $_POST['confirm_mass_delete'] != '1') {
                echo "<div style='background: #f8d7da; border: 1px solid #f5c6cb; padding: 15px; margin: 10px 
0; border-radius: 4px;'>";
                echo "<strong>❌ ERROR:</strong> Debes confirmar que quieres eliminar TODOS los productos 
marcando la casilla de confirmación.";
                echo "</div>";
                return;
            }
            
            $results = $cleaner->massDeleteAllProducts($dry_run, $batch_size);
            echo "<div style='background: #f8d7da; border: 1px solid #f5c6cb; padding: 15px; margin: 10px 0; 
border-radius: 4px;'>";
            echo "<strong>🚨 ELIMINACIÓN MASIVA COMPLETA:</strong> Se procesarán TODOS los productos de la 
tienda en lotes de {$batch_size}.";
            echo "</div>";
            ...
            $results = $cleaner->safeCleanup($product_ids, $dry_run, $batch_size);
        } else {
            $results = $cleaner->cleanupByIssueType($cleanup_type, $dry_run, $batch_size);
        }
        
        echo "<p><strong>Modo:</strong> " . ($dry_run ? "SIMULACIÓN" : "ELIMINACIÓN REAL") . "</p>";
        echo "<p><strong>Tamaño del lote:</strong> {$batch_size} productos por ejecución</p>";
        ...
        if (!empty($results['remaining_products'])) {
            $remaining_count = count($results['remaining_products']);
            echo "<div style='background: #e7f3ff; border: 1px solid #b3d9ff; padding: 10px; margin: 10px 0; 
border-radius: 4px;'>";
            echo "<strong>🔄 Productos restantes:</strong> {$remaining_count} productos por procesar. ";
            echo "Ejecuta este script nuevamente para procesar el siguiente lote.";
            echo "</div>";
        }
        ...
        if (!empty($results['deleted'])) {
            ...
            foreach ($results['deleted'] as $deleted) {
                echo "<li>ID: {$deleted['id']} - SKU: " . ($deleted['sku'] ?: 'N/A') . " - Nombre: " . 
esc_html($deleted['name']) . "</li>";
            }
            echo "</ul>";
        }
        
        if (!empty($results['skipped'])) {
            echo "<h3>⏭ Productos Omitidos</h3>";
            echo "<ul>";
            foreach ($results['skipped'] as $skipped) {
                echo "<li>ID: {$skipped['id']} - Razón: {$skipped['reason']}" . (isset($skipped['name']) ? " - 
Nombre: " . esc_html($skipped['name']) : "") . "</li>";
            }
            echo "</ul>";
        }
    }
