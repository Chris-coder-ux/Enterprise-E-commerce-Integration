...
(function($) {
  ...
  // Asegurarse de que miApiModern esté disponible
  ...
  $(document).ready(function() {
    ...
    // Handler para pruebas de compatibilidad con debounce para prevenir clics múltiples
    $('.test-compatibility').on('click', miApiModern.debounce(function() {
      const $button = $(this);
      ...
      // Usar el cliente API moderno para realizar la solicitud
      miApiModern.apiClient.post(
        type === 'theme' ? 'mi_integracion_test_theme_compatibility' : 'mi_integracion_test_plugin_compatibility',
        {
          slug: slug,
      ...
      .then(response => {
        if (response.success) {
          ...
          miApiModern.toast.success(miApiCompat.testCompleted);
        } else {
          // Mostrar error
          miApiModern.toast.error(miApiCompat.testError + ': ' + (response.message || 'Error desconocido'));
        }
      })
      .catch(error => {
        miApiModern.toast.error('Error: ' + (error.message || 'Error de conexión'));
        console.error('Error en prueba de compatibilidad:', error);
      })
      .finally(() => {
        // Restaurar el botón a su estado original
        $button.removeClass('loading').prop('disabled', false);
        $button.html(miApiCompat.runTest);
      });
    }, 300));
