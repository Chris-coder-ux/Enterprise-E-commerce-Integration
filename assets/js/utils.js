...
(function($) {
    ...
    function ajaxRequest(action, data, successCallback, errorCallback) {
        ...
        if (!miIntegracionApi || !miIntegracionApi.nonce) {
            console.error('ERROR AJAX: miIntegracionApi.nonce no está definido');
            if (typeof errorCallback === 'function') {
                errorCallback({ message: 'Error de configuración: falta el token de seguridad' });
            } else {
                showNotification('Error de configuración: falta el token de seguridad', 'error');
            }
            return;
        }

        if (!action) {
            console.error('ERROR AJAX: action no puede estar vacío');
            if (typeof errorCallback === 'function') {
                errorCallback({ message: 'Error: falta el parámetro action' });
            }
            return;
        }
        ...
        $.ajax({
            url: miIntegracionApi.ajaxUrl || ajaxurl, // Fallback a ajaxurl global
            type: 'POST',
            data: requestData,
            dataType: 'json',
