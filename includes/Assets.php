...
class Assets {
	...
	public function enqueue_admin_styles( $hook ) {
		...
		// Cargar el sistema de diseño modernizado
		wp_enqueue_style(
			$this->plugin_name . '-design-system',
			$this->assets_url . 'css/design-system.css',
		...
		wp_enqueue_style(
			...
			array($this->plugin_name . '-compatibility', $this->plugin_name . '-design-system'),
			$this->version,
			'all'
		);

		// Estilos UI de Verial (ahora depende del sistema de diseño)
		wp_enqueue_style(
			$this->plugin_name . '-verial-ui',
			$this->assets_url . 'css/verial-ui.css',
		...
		if ( strpos( $hook, 'mi-integracion-dashboard' ) !== false ) {
			wp_enqueue_style(
				...
				array( $this->plugin_name . '-admin', $this->plugin_name . '-design-system' ),
				$this->version,
				'all'
			);
		}
		...
		// ELIMINADO: Página de diagnóstico API - funcionalidad duplicada innecesaria
		...
		if ( strpos( $hook, 'mi-integracion-compatibility' ) !== false ) {
			wp_enqueue_style(
				$this->plugin_name . '-compatibility-report',
		...
		// Página de test HPOS (High-Performance Order Storage)
	...
	public function enqueue_admin_scripts( $hook ) {
		...
		// --- Script de utilidades común para todos los scripts admin---
		wp_enqueue_script(
		...
		// --- Script de utilidades modernas mejoradas ---
		wp_enqueue_script(
			$this->plugin_name . '-utils-modern',
			$this->assets_url . 'js/utils-modern.js',
		...
		wp_enqueue_script(
			...
			array( 'jquery', $this->plugin_name . '-utils', $this->plugin_name . '-utils-modern' ), // Depende de utils.js y utils-modern.js
			$this->version,
			true
		);
		...
		if ( strpos( $hook, 'mi-integracion-logs' ) !== false ) {
			wp_enqueue_script(
				...
				array( 'jquery', $this->plugin_name . '-admin-main', $this->plugin_name . '-utils', $this->plugin_name . '-utils-modern' ), 
				$this->version,
				true
			);
		...
		if ( strpos( $hook, 'mi-integracion-settings' ) !== false ) {
			wp_enqueue_script(
				...
				array( 'jquery', $this->plugin_name . '-admin-main', $this->plugin_name . '-utils-modern', 'jquery-ui-tabs' ),
				$this->version,
				true
			);
		...
		// 		array( 'jquery', $this->plugin_name . '-admin-main', $this->plugin_name . '-utils-modern' ),
		// 		$this->version,
		// 		true
		// 	);
	...
	public function enqueue_public_styles() {
		...
		// Cargar primero el sistema de diseño
		wp_enqueue_style(
			$this->plugin_name . '-design-system',
			$this->assets_url . 'css/design-system.css',
			array(),
		...
		// Luego cargar los estilos del frontend que dependen del sistema de diseño
		wp_enqueue_style(
		...
		// Cargar estilos públicos adicionales si existen
		if (file_exists($this->assets_url . 'css/public.css')) {
			wp_enqueue_style(
				$this->plugin_name . '-public',
				$this->assets_url . 'css/public.css',
				array($this->plugin_name . '-design-system', $this->plugin_name . '-frontend'),
				$this->version,
				'all'
			);
		}
	...
	public function enqueue_public_scripts() {
		...
		// --- Script de utilidades común para todos los scripts públicos ---
		wp_enqueue_script(
			$this->plugin_name . '-utils',
			$this->assets_url . 'js/utils.js',
			array( 'jquery' ),
		...
		// --- Script de utilidades modernas para el frontend ---
		wp_enqueue_script(
			$this->plugin_name . '-utils-modern',
			$this->assets_url . 'js/utils-modern.js',
		...
		wp_enqueue_script(
			...
			array( 'jquery', $this->plugin_name . '-utils', $this->plugin_name . '-utils-modern' ),
			$this->version,
			true
		);
		...
		// Cargar componentes lazy si existe
		if (file_exists($this->assets_url . 'js/lazy-components.js')) {
			wp_enqueue_script(
				$this->plugin_name . '-lazy-components',
				$this->assets_url . 'js/lazy-components.js',
				array( 'jquery', $this->plugin_name . '-frontend-main', $this->plugin_name . '-utils-modern' ),
				$this->version,
				true
			);
		}		
