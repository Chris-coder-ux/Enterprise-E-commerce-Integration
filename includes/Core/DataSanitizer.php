...
class DataSanitizer {
    ...
    public function __construct() {
        $this->logger = new Logger('data_sanitizer');
    }
    ...
    private function sanitizeText($value): string {
        ...
        // Verificar si sanitize_text_field funciona correctamente
        $sanitized = sanitize_text_field((string)$value);
        
        // Si sanitize_text_field devuelve null (como en los stubs), usar implementación alternativa
    ...
    private function sanitizeEmail($value): string {
        ...
        // Verificar si sanitize_email funciona correctamente
        $sanitized = sanitize_email((string)$value);
        
        // Si sanitize_email devuelve null (como en los stubs), usar implementación alternativa
        if (is_null($sanitized)) {
    ...
    private function sanitizeUrl($value): string {
        ...
        // Verificar si esc_url_raw funciona correctamente
        $sanitized = esc_url_raw((string)$value);
        
        // Si esc_url_raw devuelve null (como en los stubs), usar implementación alternativa
        if (is_null($sanitized)) {
    ...
    private function sanitizeSku($value): string {
        ...
        // Mantener caracteres alfanuméricos, guiones, puntos, guiones bajos
        // Eliminar solo caracteres potencialmente peligrosos
        $sanitized = preg_replace('/[^A-Za-z0-9\-\._\s]/', '', (string)$value);
    ...
    private function validateEmail($value): bool {
        ...
        // Manejar el caso donde WordPress stubs devuelven null
        if ($result === null) {
            // Fallback: usar filter_var si is_email no está disponible
    ...
    private function validateSku($value): bool {
        // Permitir caracteres alfanuméricos, guiones, puntos, guiones bajos
        // y espacios comunes en SKUs comerciales
        return preg_match('/^[A-Za-z0-9\-\._\s]+$/', trim($value)) === 1 && !empty(trim($value));
    }
