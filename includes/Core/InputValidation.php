...
class InputValidation {
	...
	/**
	 * Valida un precio de forma unificada
	 *
	 * Esta función centraliza TODA la validación de precios en el sistema,
	 * eliminando la necesidad de múltiples validadores redundantes.
	 *
	 * @param mixed $value Valor a validar
	 * @param array $rules Reglas adicionales:
	 *   - 'required' (bool): Si el precio es obligatorio
	 *   - 'status' (string): Estado del producto ('publish', 'draft', 'private')
	 *   - 'product_type' (string): Tipo de producto ('normal', 'sample', 'temporary')
	 *   - 'min_price' (float): Precio mínimo permitido (default: 0)
	 *   - 'max_price' (float): Precio máximo permitido (default: 999999.99)
	 * @return bool True si es válido
	 */
	public static function validate_precio( $value, $rules = array() ) {
		...
		$min_price = $rules['min_price'] ?? 0;
		$max_price = $rules['max_price'] ?? 999999.99;
		...
		// Validar precio = 0 según contexto
		if ( $price == 0 ) {
			// Permitir precio = 0 para productos en borrador o muestras
			if ( $status === 'draft' || $product_type === 'sample' || $product_type === 'temporary' ) {
				return true;
			}
			
			// Rechazar precio = 0 para productos publicados normales
			if ( $status === 'publish' && $product_type === 'normal' ) {
				self::add_error( self::ERROR_INVALID, __( 'El precio debe ser mayor que 0 para productos publicados.', 'mi-integracion-api' ) );
				return false;
			}
		}
		...
		if ( $price < $min_price ) {
			self::add_error( self::ERROR_RANGE, sprintf( __( 'El precio no puede ser menor que %s.', 'mi-integracion-api' ), $min_price ) );
			return false;
		}

		if ( $price > $max_price ) {
			self::add_error( self::ERROR_RANGE, sprintf( __( 'El precio no puede ser mayor que %s.', 'mi-integracion-api' ), $max_price ) );
			return false;
		}
