...
class JWT_Manager {
	...
	private function initialize() {
		...
		if ( defined( 'MiIntegracionApi_JWT_SECRET' ) ) {
			$this->secret_key = constant( 'MiIntegracionApi_JWT_SECRET' );
		} else {
		...
		if ( defined( 'MiIntegracionApi_JWT_EXPIRATION' ) ) {
			$this->expiration_time = (int) constant( 'MiIntegracionApi_JWT_EXPIRATION' );
		}

		if ( defined( 'MiIntegracionApi_JWT_ISSUER' ) ) {
			$this->issuer = constant( 'MiIntegracionApi_JWT_ISSUER' );
		}

		if ( defined( 'MiIntegracionApi_JWT_AUDIENCE' ) ) {
			$this->audience = constant( 'MiIntegracionApi_JWT_AUDIENCE' );
		}
	}
	...
	private function get_or_generate_secret_key() {
		...
		if ( ! $secret_key ) {
			...
			if ( function_exists( 'random_bytes' ) ) {
				...
				$secret_key = md5( uniqid( (string)mt_rand(), true ) ) . md5( uniqid( (string)mt_rand(), true ) );
			}
