<?php
    ...
    class LazyLoader
    {
        ...
        public static function init()
        {
            ...
            add_filter('wp_get_attachment_image_attributes', [__CLASS__, 'add_lazyload_to_images'], 10, 3);

            // Añadir script auxiliar para carga diferida
            add_action('admin_footer', [__CLASS__, 'add_lazyload_script']);
            add_action('wp_footer', [__CLASS__, 'add_lazyload_script']);
        }
        ...
        {
            if (defined('MI_DISABLE_LAZYLOAD') && constant('MI_DISABLE_LAZYLOAD')) {
                return;
            }
...
(function() {
	...
	window.miIntegracionApiLazyLoader = {
		...
		<?php
                 ...
                 */
                public static function lazy_component($content, $callback = '', $attributes = [])
                {
                 ...
                 */
                public static function register_lazy_script($handle, $src, $deps = [], $ver = false, $in_footer = true)
                {
                    wp_register_script($handle, $src, $deps, $ver, $in_footer);
                    self::$lazy_scripts[] = $handle;
                    // Inyectar función JS para carga bajo demanda solo una vez
                    add_action('admin_footer', [__CLASS__, 'inject_lazy_loader_js'], 1);
                    add_action('wp_footer', [__CLASS__, 'inject_lazy_loader_js'], 1);
                }
