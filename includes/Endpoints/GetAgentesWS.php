...
class GetAgentesWS extends \MiIntegracionApi\Endpoints\Base {
	 ...
	 * @return array<int, array<string, mixed>>|\MiIntegracionApi\ErrorHandling\Responses\SyncResponseInterface Array de agentes o error
	 */
	protected function format_verial_response( array $verial_response ) {
		...
		foreach ( $verial_response['Agentes'] as $agente_verial ) {
			$agentes[] = array(
				'id'     => isset( $agente_verial['Id'] ) ? (int) $agente_verial['Id'] : null,
	...
	public function execute_restful( \WP_REST_Request $request ): \WP_REST_Response|\MiIntegracionApi\ErrorHandling\Responses\SyncResponseInterface {
		...
		$this->set_cached_data( (string)$cache_params_for_key, $formatted_response );
