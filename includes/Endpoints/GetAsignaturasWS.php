...
class GetAsignaturasWS extends \MiIntegracionApi\Endpoints\Base {
	...
	protected function format_verial_response( array $verial_response ) {
		...
		if ( isset( $verial_response['Valores'] ) && is_array( $verial_response['Valores'] ) ) {
			...
			$error_message = __( 'Los datos de asignaturas recibidos de Verial no tienen el formato esperado (se esperaba la clave "Valores").', 'mi-integracion-api' );
			return new \MiIntegracionApi\ErrorHandling\Responses\SyncResponse(
				false,
		...
		foreach ( $verial_response[ $data_key ] as $asignatura_verial ) {
			$asignaturas[] = array(
				'id'     => isset( $asignatura_verial['Id'] ) ? (int) $asignatura_verial['Id'] : null,
	...
	public function execute_restful( \WP_REST_Request $request ): \WP_REST_Response|\MiIntegracionApi\ErrorHandling\Responses\SyncResponseInterface {
		...
		if ( $formatted instanceof \MiIntegracionApi\ErrorHandling\Responses\SyncResponseInterface && !$formatted->isSuccess() ) {
