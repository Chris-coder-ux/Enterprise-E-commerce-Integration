...
class GetColeccionesWS extends \MiIntegracionApi\Endpoints\Base {
	...
	protected function format_verial_response( array $verial_response ): \MiIntegracionApi\ErrorHandling\Responses\SyncResponseInterface {
		...
		if ( isset( $verial_response['Valores'] ) && is_array( $verial_response['Valores'] ) ) {
			...
			$error_message = __( 'Los datos de colecciones recibidos de Verial no tienen el formato esperado (se esperaba la clave "Valores").', 'mi-integracion-api' );
			return new \MiIntegracionApi\ErrorHandling\Responses\SyncResponse(
				false,
		...
		foreach ( $verial_response[ $data_key ] as $coleccion_verial ) {
			$colecciones[] = array(
				'id'     => isset( $coleccion_verial['Id'] ) ? (int) $coleccion_verial['Id'] : null,
		...
		return \MiIntegracionApi\ErrorHandling\Handlers\ResponseFactory::success(
			$colecciones,
			__( 'Colecciones obtenidas correctamente.', 'mi-integracion-api' ),
			[
				'endpoint' => self::ENDPOINT_NAME,
				'items_count' => count( $colecciones ),
				'timestamp' => time()
			]
		);
	}
