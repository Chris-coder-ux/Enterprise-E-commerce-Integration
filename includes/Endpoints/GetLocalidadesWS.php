...
class GetLocalidadesWS extends \MiIntegracionApi\Endpoints\Base {
	...
	protected function format_verial_response( array $verial_response ): \MiIntegracionApi\ErrorHandling\Responses\SyncResponseInterface {
		if ( ! isset( $verial_response['Localidades'] ) || ! is_array( $verial_response['Localidades'] ) ) {
			...
			return \MiIntegracionApi\ErrorHandling\Handlers\ResponseFactory::error(
				...
				[
					...
					'error_code' => 'verial_api_malformed_localidades_data',
		...
		foreach ( $verial_response['Localidades'] as $localidad_verial ) {
			$localidades[] = array(
				'id'                   => isset( $localidad_verial['Id'] ) ? (int) $localidad_verial['Id'] : null,
				'nombre'               => isset( $localidad_verial['Nombre'] ) ? sanitize_text_field( $localidad_verial['Nombre'] ) : null,
				'id_provincia'         => isset( $localidad_verial['ID_Provincia'] ) ? (int) $localidad_verial['ID_Provincia'] : null,
				'id_pais'              => isset( $localidad_verial['ID_Pais'] ) ? (int) $localidad_verial['ID_Pais'] : null,
		...
		return \MiIntegracionApi\ErrorHandling\Handlers\ResponseFactory::success(
			$localidades,
			__( 'Localidades obtenidas correctamente.', 'mi-integracion-api' ),
			[
				'endpoint' => self::ENDPOINT_NAME,
				'items_count' => count( $localidades ),
				'timestamp' => time()
			]
		);
	}
