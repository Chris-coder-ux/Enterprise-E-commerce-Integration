...
class NuevoClienteWS extends Base {
	...
	public function permissions_check( \WP_REST_Request $request ): bool|\MiIntegracionApi\ErrorHandling\Responses\SyncResponseInterface {
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			$error_message = esc_html__( 'No tienes permiso para realizar esta acción.', 'mi-integracion-api' );
	...
	public function get_endpoint_args( bool $is_update = false ): array {
		$args = array(
			...
			'Apellido1'        => array(
				...
				'sanitize_callback' => 'sanitize_text_field',
				'validate_callback' => function ( $param ) {
					return is_string( $param ) && mb_strlen( $param ) <= VerialTypes::MAX_LENGTH_APELLIDO;
				},
			),
			'Apellido2'        => array(
			...
			'RazonSocial'      => array(
				...
				'sanitize_callback' => 'sanitize_text_field',
				'validate_callback' => function ( $param ) {
					return is_string( $param ) && mb_strlen( $param ) <= VerialTypes::MAX_LENGTH_RAZON_SOCIAL;
				},
			),
			'RegFiscal'        => array(
			...
			'Provincia'        => array(
				...
				'sanitize_callback' => 'sanitize_text_field',
				'validate_callback' => function ( $param ) {
					return is_string( $param ) && mb_strlen( $param ) <= VerialTypes::MAX_LENGTH_PROVINCIA_MANUAL;
				},
			),
			'ID_Localidad'     => array(
			...
			'Localidad'        => array(
				...
				'sanitize_callback' => 'sanitize_text_field',
				'validate_callback' => function ( $param ) {
					return is_string( $param ) && mb_strlen( $param ) <= VerialTypes::MAX_LENGTH_LOCALIDAD_MANUAL;
				},
			),
			'LocalidadAux'     => array(
				...
				'sanitize_callback' => 'sanitize_text_field',
				'validate_callback' => function ( $param ) {
					return is_string( $param ) && mb_strlen( $param ) <= VerialTypes::MAX_LENGTH_LOCALIDAD_AUX;
				},
			),
			'CPostal'          => array(
				...
				'sanitize_callback' => 'sanitize_text_field',
				'validate_callback' => function ( $param ) {
					return is_string( $param ) && mb_strlen( $param ) <= VerialTypes::MAX_LENGTH_CP;
				},
			),
			'Direccion'        => array(
				...
				'sanitize_callback' => 'sanitize_text_field',
				'validate_callback' => function ( $param ) {
					return is_string( $param ) && mb_strlen( $param ) <= VerialTypes::MAX_LENGTH_DIRECCION;
				},
			),
			'Telefono'         => array(
				...
				'sanitize_callback' => 'sanitize_text_field',
				'validate_callback' => function ( $param ) {
					return is_string( $param ) && mb_strlen( $param ) <= VerialTypes::MAX_LENGTH_TELEFONO;
				},
			),
			'Telefono1'        => array(
			...
			'Email'            => array(
				...
				'validate_callback' => function ( $param ) {
					return is_string( $param ) && mb_strlen( $param ) <= VerialTypes::MAX_LENGTH_EMAIL;
				},
			),
			'Sexo'             => array(
			...
			'WebUserOld'       => array(
				...
				'sanitize_callback' => 'sanitize_text_field',
				'validate_callback' => function ( $param ) {
					return is_string( $param ) && mb_strlen( $param ) <= VerialTypes::MAX_LENGTH_WEB_USER;
				},
			),
			'WebUser'          => array(
			...
			'WebPassword'      => array(
				...
				'type'        => 'string',
				'required'    => false,
				'maxLength'   => VerialTypes::MAX_LENGTH_WEB_PASS,
			),
	...
	public static function get_cliente_properties_args(): array {
		// Crear una instancia temporal con un ApiConnector dummy para obtener los argumentos
		$dummy_connector = new \MiIntegracionApi\Core\ApiConnector();
		$args = ( new self( $dummy_connector ) )->get_endpoint_args( false );
	...
	public function execute_restful( \WP_REST_Request $request ): \WP_REST_Response|\MiIntegracionApi\ErrorHandling\Responses\SyncResponseInterface {
		...
		if ( ! empty( $params['ID_Provincia'] ) ) {
			$verial_payload['ID_Provincia'] = (int) $params['ID_Provincia'];
		} elseif ( ! empty( $params['Provincia'] ) ) {
			$verial_payload['Provincia'] = $params['Provincia'];
		}
		// Localidad
		if ( ! empty( $params['ID_Localidad'] ) ) {
			$verial_payload['ID_Localidad'] = (int) $params['ID_Localidad'];
		...
		// Verificar si la respuesta es un error procesado
		if ( is_array( $verial_response ) && isset( $verial_response['success'] ) && $verial_response['success'] === false ) {
			$error_message = $verial_response['message'] ?? 'Error desconocido de Verial';
			return rest_ensure_response( new \MiIntegracionApi\ErrorHandling\Responses\SyncResponse(
				false,
				['verial_response' => $verial_response],
				new \MiIntegracionApi\ErrorHandling\Exceptions\SyncError(
					$verial_response['error'] ?? 'verial_error',
					$error_message,
					['status' => 400]
				),
				400,
		...
		// Si es un número, es el ID del cliente creado
		if ( is_numeric( $verial_response ) ) {
			return rest_ensure_response( $this->format_success_response( null, [
				'message' => __( 'Cliente creado en Verial con éxito.', 'mi-integracion-api' ),
				'id_cliente_verial' => intval( $verial_response )
			] ) );
		}

		// Respuesta estándar de éxito para otros casos
	 ...
	 * @return array|\MiIntegracionApi\ErrorHandling\Responses\SyncResponseInterface Respuesta de Verial o error.
	 */
	protected function send_to_verial( array $data, array $request_params ): array|\MiIntegracionApi\ErrorHandling\Responses\SyncResponseInterface {
		// Preparar datos para Verial
		$verial_data = $this->prepare_data_for_verial( $data );

		// Llamada real a la API de Verial
		try {
			$response = $this->connector->call( 'NuevoClienteWS', 'POST', $verial_data );
			
			// El método call ya procesa la respuesta con process_new_customer_response
			// y devuelve el ID del cliente o un array de error
			return $response;
			
		} catch ( \Exception $e ) {
			$this->logger->error( 'Error en llamada a NuevoClienteWS', [
				'error' => $e->getMessage(),
				'data' => $verial_data
			] );
			
			$error_message = 'Error al crear cliente en Verial: ' . $e->getMessage();
			return new \MiIntegracionApi\ErrorHandling\Responses\SyncResponse(
				false,
				['exception' => $e->getMessage(), 'data' => $verial_data],
				new \MiIntegracionApi\ErrorHandling\Exceptions\SyncError(
					'verial_api_error',
					$error_message,
					['status' => 500]
