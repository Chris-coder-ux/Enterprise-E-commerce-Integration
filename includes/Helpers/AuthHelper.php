 ...
 * Helper para autenticación adicional (API key/JWT) en endpoints REST.
 *
 * Permite validar API key en header o JWT en Authorization, aplicar rate limiting y registrar eventos de seguridad.
 * Incluye utilidades para obtener la IP real, validar y revocar tokens, y alertar al administrador en caso de abuso.
 *
 * @package MiIntegracionApi\Helpers
 */
class AuthHelper {
     /**
      * Número máximo de intentos permitidos en la ventana de tiempo para rate limiting.
      */
     const RATE_LIMIT_MAX_ATTEMPTS = 20;

    /**
     * Duración de la ventana de tiempo (en segundos) para el rate limiting.
     */
    const RATE_LIMIT_WINDOW = 60;

    /**
     * Tiempo de bloqueo (en segundos) tras exceder el límite de intentos.
     */
    const RATE_LIMIT_BLOCK_TIME = 600;

    /**
     * Valida la autenticación adicional para un request REST (API key o JWT).
     *
     * Devuelve SyncResponseInterface con el resultado de la validación.
     *
     * @param \WP_REST_Request $request Solicitud REST a validar.
     * @return \MiIntegracionApi\ErrorHandling\Responses\SyncResponseInterface
     */
    public static function validate_rest_auth($request): \MiIntegracionApi\ErrorHandling\Responses\SyncResponseInterface {
        ...
        if ($api_key && self::is_valid_api_key($api_key)) {
            return \MiIntegracionApi\ErrorHandling\Handlers\ResponseFactory::success(
                ...
                [
                    'endpoint' => 'AuthHelper::validate_rest_auth',
                    'auth_method' => 'api_key',
                    'timestamp' => time()
                ]
            );
        ...
        if ($auth_header && preg_match('/Bearer\s+(.*)$/i', $auth_header, $matches)) {
            ...
            if (self::is_valid_jwt($jwt)) {
                return \MiIntegracionApi\ErrorHandling\Handlers\ResponseFactory::success(
                    ['auth_method' => 'jwt', 'jwt' => $jwt],
                    'Autenticación con JWT exitosa',
                    [
                        'endpoint' => 'AuthHelper::validate_rest_auth',
                        'auth_method' => 'jwt',
                        'timestamp' => time()
    ...
    /**
     * Obtiene la IP real del cliente considerando proxies y cabeceras comunes.
     *
     * @return string IP real del cliente o 'unknown' si no se puede determinar.
     */
    ...
    /**
     * Valida la API key recibida contra las almacenadas en opciones o variables de entorno.
     *
     * Registra eventos de seguridad si la clave es inválida o revocada.
     *
     * @param string $api_key API key a validar.
     * @return bool True si la API key es válida y no está revocada, false en caso contrario.
     */
    ...
    /**
     * Registra eventos de seguridad usando el sistema de logging del plugin.
     *
     * @param string $event_type Tipo de evento de seguridad.
     * @param array  $context    Contexto adicional del evento (opcional).
     * @return void
     */
    ...
    /**
     * Valida el JWT recibido con comprobaciones avanzadas (expiración, audiencia, emisor, revocación).
     *
     * Registra eventos de seguridad si el JWT es inválido, está expirado, revocado o la librería falta.
     *
     * @param string $jwt JWT a validar.
     * @return bool True si el JWT es válido y no está revocado, false en caso contrario.
     */
    ...
    /**
     * Comprueba si un JWT está revocado (presente en la lista de tokens revocados).
     *
     * @param string $jwt JWT a comprobar.
     * @return bool True si el JWT está revocado, false en caso contrario.
     */
     ...
     *
     * Si se excede el límite, bloquea temporalmente y alerta al administrador.
     *
     * @param string      $context Contexto o endpoint a limitar.
     * @param string|null $api_key API key asociada (opcional).
     * @return \MiIntegracionApi\ErrorHandling\Responses\SyncResponseInterface
     */
    public static function check_rate_limit($context, $api_key = null): \MiIntegracionApi\ErrorHandling\Responses\SyncResponseInterface {
        if (empty($context)) {
            return \MiIntegracionApi\ErrorHandling\Handlers\ResponseFactory::error(
                esc_html__('Contexto inválido para rate limiting.', 'mi-integracion-api'),
                400,
                [
                    'endpoint' => 'AuthHelper::check_rate_limit',
        ...
        return \MiIntegracionApi\ErrorHandling\Handlers\ResponseFactory::success(
            [
                ...
                'ip' => $ip,
                'count' => $data['count'],
                'remaining_attempts' => self::RATE_LIMIT_MAX_ATTEMPTS - $data['count']
            ],
    ...
    /**
     * Envía una alerta por email al administrador si se detecta abuso de rate limiting.
     *
     * También registra el evento en el log del plugin.
     *
     * @param string $context Contexto o endpoint donde se detectó el abuso.
     * @param string $ip      IP del cliente.
     * @param string $key     API key/JWT parcial (si aplica).
     * @param array  $data    Datos del rate limit (intentos, bloqueo, etc).
     * @return void
     */
