 ...
 *
 * Permite crear la tabla de logs, registrar logs y hooks de activación, y mantener compatibilidad con código antiguo.
 *
 * @package MiIntegracionApi\Helpers
 * @since 1.0.0
 */
class DbLogs {
     ...
     *
     * Utiliza el Installer centralizado si está disponible. Registra logs de éxito o error según corresponda.
     *
     * @return bool True si la tabla fue creada o actualizada correctamente, false en caso contrario.
     */
    public static function crear_tabla_logs() {
        ...
        if (class_exists('MiIntegracionApi\\Core\\Installer')) {
            $result = \MiIntegracionApi\Core\Installer::create_logs_table();
            if ($result) {
                // Log de éxito
                if (class_exists('MiIntegracionApi\\Helpers\\Logger')) {
                    $logger = new \MiIntegracionApi\Helpers\Logger('db_logs');
                    $logger->info('Tabla de logs creada usando Installer centralizado');
                }
            }
            return $result;
        } else {
            ...
            if (class_exists('MiIntegracionApi\\Helpers\\Logger')) {
                $logger = new \MiIntegracionApi\Helpers\Logger('db_logs');
                $logger->error('Clase Installer no encontrada, no se puede crear la tabla de logs');
            }
            return false;
        }
    }
    ...
    /**
     * Registra el hook de activación para crear la tabla de logs al activar el plugin.
     *
     * @return void
     */
     ...
     *
     * Inserta un registro en la tabla de logs con los datos proporcionados, sanitizando y validando los campos.
     *
     ...
     * @param array  $contexto Datos adicionales (opcional).
