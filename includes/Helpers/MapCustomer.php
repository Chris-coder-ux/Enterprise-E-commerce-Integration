 ...
 *
 * Prepara el payload para el endpoint NuevoClienteWS de Verial y realiza validaciones y sanitización de datos.
 * Incluye funciones para convertir entre formatos de WooCommerce y Verial, y compatibilidad legacy.
 *
 ...
 *
 * Proporciona métodos estáticos para convertir datos de clientes entre los formatos de WooCommerce y Verial,
 * aplicando validaciones y sanitización de datos críticos.
 *
 * @since 1.0.0
 * @package MiIntegracionApi\Helpers
 */
class MapCustomer {
    ...
    public static function init() {
        if (!self::$logger) {
            self::$logger = new Logger('map_customer');
        }
        if (!self::$sanitizer) {
     ...
     *
     * Valida y sanitiza los datos del cliente de Verial y los convierte a un objeto CustomerDTO compatible con WooCommerce.
     *
     * @param array $verial_customer Datos del cliente de Verial.
     * @return CustomerDTO|null DTO del cliente mapeado o null si los datos son inválidos o faltan campos críticos.
     */
     ...
     *
     * Convierte y sanitiza los datos de un objeto WC_Customer al formato requerido por Verial.
     *
     * @param \WC_Customer $wc_customer Cliente de WooCommerce.
     * @return array Datos del cliente en formato Verial. Devuelve array vacío si los datos son inválidos.
     */
    public static function wc_to_verial(\WC_Customer $wc_customer): array {
        self::init();
     ...
     *
     * @deprecated Usa MapCustomer::wc_to_verial() en su lugar.
     * @param \WC_Customer $customer Cliente de WooCommerce.
     * @param array $additional_data Datos adicionales (no utilizados en la nueva versión).
     * @return array Datos del cliente en formato Verial.
     */
    public static function map_customer_to_verial(\WC_Customer $customer, array $additional_data = []): array {
