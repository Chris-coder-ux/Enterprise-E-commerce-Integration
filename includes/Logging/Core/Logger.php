 ...
 * @version 1.2.0
 */
class LoggerBasic implements ILogger
{
    /**
    ...
    /**
     * Instancias singleton del logger por categoría.
     *
     * @var array<string, Logger> Mapa de categorías a instancias de logger.
     */
    ...
    /**
     * Obtiene una instancia singleton del logger para la categoría especificada.
     *
     * @param string $category Categoría del logger.
     * @param string $minLevel Nivel mínimo de log (opcional).
     * @return LoggerBasic Instancia del logger.
     * @since 1.2.0
     */
    public static function getInstance(string $category = 'default', string $minLevel = self::LEVEL_DEBUG): LoggerBasic
    {
        if (!isset(self::$instances[$category])) {
            self::$instances[$category] = new self($category, $minLevel);
        }
