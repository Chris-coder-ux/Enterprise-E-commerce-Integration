...
class Security {
	...
	public function extend_nonce_lifetime( $lifetime ) {
		...
		if ( str_starts_with( $current_action, MiIntegracionApi_OPTION_PREFIX ) ||
			str_starts_with( $current_action, str_replace( '_', '-', MiIntegracionApi_OPTION_PREFIX ) ) ) {
	 ...
	 * @return   SyncResponseInterface|boolean  SyncResponseInterface si $die=false, boolean si $die=true.
	 */
	public function verify_nonce( $nonce, $action, $die = true ) {
		// Prepender prefijo si no está presente
		if ( !str_starts_with( $action, MiIntegracionApi_NONCE_PREFIX ) ) {
		...
		// Si no es válido
		if (!$valid) {
			// Si se solicita terminar ejecución, usar wp_die (comportamiento original)
			if ($die) {
				wp_die(
			...
			// Si no se debe terminar, devolver SyncResponseInterface
			return ResponseFactory::error(
				__('Error de seguridad: Nonce inválido. Por favor, recarga la página e intenta nuevamente.', MiIntegracionApi_TEXT_DOMAIN),
		...
		// Si es válido y no se debe terminar, devolver éxito
		if (!$die) {
			return ResponseFactory::success(
				['nonce' => $nonce, 'action' => $action],
				__('Nonce verificado correctamente', MiIntegracionApi_TEXT_DOMAIN),
				[
					'action' => $action,
					'user_id' => get_current_user_id()
				]
	...
	public function verify_capability( $capability, $object_id = null, $die = true ) {
		...
		if (!$user_id) {
			...
			// Si no se debe terminar, devolver SyncResponseInterface
			return ResponseFactory::error(
				__('Necesitas iniciar sesión para realizar esta acción.', MiIntegracionApi_TEXT_DOMAIN),
		...
		if (!$has_capability) {
			...
			// Si no se debe terminar, devolver SyncResponseInterface
			return ResponseFactory::error(
				__('No tienes permisos para realizar esta acción.', MiIntegracionApi_TEXT_DOMAIN),
				HttpStatusCodes::FORBIDDEN,
				[
					'capability' => $capability,
					'object_id' => $object_id,
		...
		// Si tiene capacidad y no se debe terminar, devolver éxito
		if (!$die) {
			return ResponseFactory::success(
				['capability' => $capability, 'object_id' => $object_id, 'user_id' => $user_id],
				__('Permisos verificados correctamente', MiIntegracionApi_TEXT_DOMAIN),
				[
					'capability' => $capability,
					'object_id' => $object_id,
					'user_id' => $user_id
	 ...
	 * @return   SyncResponseInterface  Respuesta con datos sanitizados o errores de validación.
	 */
	public function validate_form_data( $data, $rules ): SyncResponseInterface {
		$sanitized = array();
		$errors    = array();
		...
		// Devolver resultado como SyncResponseInterface
		if (empty($errors)) {
			return ResponseFactory::success(
				$sanitized,
				__('Datos validados correctamente', MiIntegracionApi_TEXT_DOMAIN),
				[
					'fields_validated' => count($sanitized),
					'validation_rules' => array_keys($rules)
				]
			);
		} else {
			return ResponseFactory::error(
				__('Error de validación en los datos del formulario', MiIntegracionApi_TEXT_DOMAIN),
	...
	/**
	 * Verifica un nonce de WordPress (método de compatibilidad).
	 * 
	 * @since 1.0.0
	 * @param    string  $nonce         El valor del nonce a verificar.
	 * @param    string  $action        Acción asociada al nonce.
	 ...
	 */
	public function verify_nonce_legacy( $nonce, $action, $die = true ) {
		$result = $this->verify_nonce($nonce, $action, $die);
		
		// Si es SyncResponseInterface, convertir a boolean
	...
	/**
	 * Verifica si un usuario tiene los permisos necesarios (método de compatibilidad).
	 * 
	 * @since 1.0.0
	 * @param    string  $capability     Capacidad requerida.
	 * @param    int     $object_id      ID del objeto para capacidades específicas.
	 ...
	 */
	public function verify_capability_legacy( $capability, $object_id = null, $die = true ) {
		$result = $this->verify_capability($capability, $object_id, $die);
	...
	/**
	 * Sanitiza y valida datos de formulario (método de compatibilidad).
	 * 
	 * @since 1.0.0
	 * @param    array $data          Datos a validar.
	 * @param    array $rules         Reglas de validación.
	 * @return   array                    Datos sanitizados o errores (formato legacy).
	 */
	public function validate_form_data_legacy( $data, $rules ) {
		$result = $this->validate_form_data($data, $rules);
		
		// Convertir SyncResponseInterface a array legacy
		if ($result->isSuccess()) {
			return [
				'sanitized' => $result->getData(),
				'errors' => [],
				'is_valid' => true
			];
		} else {
	 ...
	 *
	 * @since 1.0.0
	 * @param    mixed $errors    Errores existentes (compatibilidad con WordPress).
	 * @return   \MiIntegracionApi\ErrorHandling\Responses\SyncResponseInterface|null
	 */
	public function validate_rest_access( $errors ) {
		// Si ya hay errores de WordPress, convertirlos a SyncResponseInterface
		if ( is_wp_error( $errors ) ) {
			return ResponseFactory::error(
				$errors->get_error_message(),
				$errors->get_error_data()['status'] ?? HttpStatusCodes::INTERNAL_SERVER_ERROR,
				$errors->get_error_data() ?? [],
				['error_code' => $errors->get_error_code(), 'source' => 'wordpress_error']
			);
		}

		// Solo procesar para endpoints del plugin
		if ( strpos( $_SERVER['REQUEST_URI'], '/' . MiIntegracionApi_TEXT_DOMAIN . '/' ) === false ) {
			return null; // Continuar con la autenticación estándar de WordPress
		}
		...
		if ( str_starts_with( $auth_header, 'Bearer ' ) ) {
			...
			if ( class_exists( 'MiIntegracionApi\\Helpers\\AuthHelper' ) ) {
				...
				return ResponseFactory::success(
					['auth_method' => 'jwt', 'token' => $token],
					'Acceso autorizado con JWT',
					[
						'endpoint' => 'Security::validate_rest_access',
						'auth_method' => 'jwt',
		...
		if (!empty($api_key)) {
			...
			if ( $api_key === $valid_api_key ) {
				...
				return ResponseFactory::success(
					['auth_method' => 'api_key', 'api_key' => $api_key],
		...
		// Continuar con la autenticación estándar de WordPress
		return null;
	}
	...
	protected function is_public_endpoint( $path ) {
		...
		foreach ( $public_endpoints as $endpoint ) {
			if ( str_contains( $path, '/' . MiIntegracionApi_TEXT_DOMAIN . '/v1/' . $endpoint ) ) {
				return true;
			}
		}
	 ...
	 */
	public function validate_auth_token( $token ): \MiIntegracionApi\ErrorHandling\Responses\SyncResponseInterface {
		if (!class_exists('MiIntegracionApi\\Helpers\\AuthHelper')) {
			return ResponseFactory::error(
				'AuthHelper no está disponible',
				HttpStatusCodes::INTERNAL_SERVER_ERROR,
				[
					'endpoint' => 'Security::validate_auth_token',
					'error_code' => 'auth_helper_unavailable',
					'token' => $token,
					'timestamp' => time()
				]
			);
		...
		$result = MiIntegracionApi\Helpers\AuthHelper::validate_jwt( $token );
