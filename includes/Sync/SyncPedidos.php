...
class SyncPedidos extends BatchProcessor {
	...
	public function __construct()
	{
		...
		$this->logger = new Logger('sync_pedidos');
	}
	...
	public static function process_retry_queue( \MiIntegracionApi\Core\ApiConnector $api_connector ) {
		...
		foreach ( $queue as $order_id => $info ) {
			...
			$payload_documento_verial = \MiIntegracionApi\Helpers\MapOrder::wc_order_to_verial_nuevo_doc_payload( $order, array(), null );
			$response_verial          = $api_connector->post( 'NuevoDocClienteWS', $payload_documento_verial );
					if ( is_array($response_verial) && isset($response_verial['success']) && $response_verial['success'] === false ) {
						self::add_to_retry_queue( $order_id, $response_verial['error'] ?? ($response_verial['message'] ?? 'Error desconocido') );
						continue;
	...
	public static function sync( 
		...
		if (!class_exists('WooCommerce')) {
			throw new \MiIntegracionApi\ErrorHandling\Exceptions\SyncError(
			...
			foreach ( $order_ids as $order_id ) {
				...
				$response_verial = $api_connector->post( 'NuevoDocClienteWS', $payload_documento_verial );
						if ( is_array($response_verial) && isset($response_verial['success']) && $response_verial['success'] === false ) {
							...
							$error_code = $response_verial['error_code'] ?? '';
							$error_message = $response_verial['error'] ?? ($response_verial['message'] ?? 'Error desconocido');
							
							// Manejo específico para timeout
							if ( strpos( $error_code, 'timeout' ) !== false ) {
								$timeout_error = SyncError::timeoutError(
									'Timeout en sincronización de pedido',
									[
										'order_id' => $order_id,
								...
								self::add_to_retry_queue( $order_id, $timeout_error->getMessage() );
							} elseif ( strpos( $error_code, 'http_error_5' ) !== false || strpos( $error_code, 'connection' ) !== false ) {
								self::add_to_retry_queue( $order_id, $error_message );
							}
							++$errors;

							$error_msg = sprintf( __( 'Error al sincronizar pedido ID %1$d con Verial: %2$s', 'mi-integracion-api' ), $order->get_id(), $error_message );
							$log[]     = $error_msg;
	...
	public static function sync_batch( \MiIntegracionApi\Core\ApiConnector $api_connector, array $order_ids = array(), array $filters = array(), bool $force_restart = false ) {
			if ( ! class_exists( 'WooCommerce' ) ) {
				return array(
					'success' => false,
					'error' => __( 'WooCommerce no está activo.', 'mi-integracion-api' ),
			...
			if ( ! class_exists( 'MI_Sync_Lock' ) || ! MI_Sync_Lock::acquire() ) {
				return array(
					'success' => false,
					'error' => __( 'Ya hay una sincronización en curso.', 'mi-integracion-api' ),
		...
		try {
			...
			$batcher = \MiIntegracionApi\Core\ProductBatchProcessor::getInstance();
			...
			if ( $es_modificable ) {
				...
				if ( $referencia ) {
					$payload_update['Referencia'] = $referencia;
						...
						if ( is_array($resp_update) && isset($resp_update['success']) && $resp_update['success'] === false ) {
							throw new \RuntimeException( sprintf( 
								__( 'Error al actualizar pedido ID %1$d en Verial: %2$s', 'mi-integracion-api' ), 
								$order->get_id(), 
								$resp_update['error'] ?? ($resp_update['message'] ?? 'Error desconocido')
							) );
		...
		// Enviar a Verial
				...
				if ( is_array($response_verial) && isset($response_verial['success']) && $response_verial['success'] === false ) {
					throw new \RuntimeException( sprintf( 
						...
						$order->get_id(), 
						$response_verial['error'] ?? ($response_verial['message'] ?? 'Error desconocido')
					) );
				} elseif ( isset( $response_verial['InfoError']['Codigo'] ) && intval( $response_verial['InfoError']['Codigo'] ) === 0 ) {
	...
	public function sync_pedido($pedido) {
		// ✅ MIGRADO: Usar IdGenerator para operation ID de pedido
		$operation_id = \MiIntegracionApi\Helpers\IdGenerator::generateOperationId('pedido_sync');
		...
		if (class_exists('\\MiIntegracionApi\\Core\\RetryManager')) {
			...
			try {
				return $retry_manager->executeWithRetry(
					$operation,
					$context['operation_id'] ?? 'sync_pedidos_operation',
					$context,
					'sync_orders' // Tipo de operación para política específica
				);
	...
	/**
	 * Procesa un lote de pedidos usando la implementación base
	 * 
	 ...
	 * Procesa un pedido individual
	 * 
	 * @param mixed $pedido Datos del pedido
	 * @return array<string, mixed> Resultado del procesamiento
	 * @throws SyncError Si ocurre un error durante el procesamiento
	 */
	protected function processItem($pedido): array
	{
