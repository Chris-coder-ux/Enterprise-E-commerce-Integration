...
class HposValidation {
	...
	public static function validate_meta_wrappers() {
		...
		// SEGURIDAD: Validar que $meta_keys sea un array válido
		if ( ! is_array( $meta_keys ) || empty( $meta_keys ) ) {
			$result['status']  = 'error';
			$result['message'] = __( 'Meta keys inválidas o vacías para validación.', 'mi-integracion-api' );
			$result['issues'][] = array(
				'type'     => 'error',
				'message'  => __( 'Array de meta keys inválido o vacío.', 'mi-integracion-api' ),
				'solution' => __( 'Verificar que el array de meta keys esté correctamente definido.', 'mi-integracion-api' ),
			);
			return $result;
		}

		// SEGURIDAD: Sanitizar cada meta key
		$sanitized_meta_keys = array_map( 'sanitize_text_field', $meta_keys );

		// SEGURIDAD: Crear placeholders para la consulta preparada
		$placeholders = implode( ',', array_fill( 0, count( $sanitized_meta_keys ), '%s' ) );

		// SEGURIDAD: Consulta preparada segura usando $wpdb->prepare()
		$query = $wpdb->prepare(
	...
	public static function migrate_order_meta( $dry_run = true ) {
		...
		if ( ! is_array( $meta_keys ) || empty( $meta_keys ) ) {
			return array(
				'status'  => 'error',
				'message' => __( 'Meta keys inválidas o vacías para migración.', 'mi-integracion-api' ),
				'total'    => 0,
				'migrated' => 0,
				'errors'   => 0,
			);
